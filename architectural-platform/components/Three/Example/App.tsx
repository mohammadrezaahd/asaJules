...
export default function App({ 
  ...
  const [materialMode, setMaterialMode] = useState<'solid' | 'wireframe'>(initialConfig?.materialMode ?? 'solid');
  const [shadows, setShadows] = useState(initialConfig?.shadows ?? true);
  const [cameraMode, setCameraMode] = useState<'perspective' | 'orthographic'>(initialConfig?.cameraMode ?? 'perspective');
  ...
  useEffect(() => {
    const handleWheel = (event: WheelEvent) => {
      ...
      if (isMouseOverRef.current) {
        ...
        const newScale = Math.max(0.1, Math.min(10, controls.scale + scaleChange));
  ...
  const handleSetControls = (newControls: Controls | ((prev: Controls) => Controls)) => {
    setControls(prev => {
      const finalControls = typeof newControls === 'function' ? newControls(prev) : newControls;
      // Notify parent if callback is provided
      if (onConfigChange) {
        onConfigChange({
          ...finalControls,
